...
let { src, dest, watch, series, parallel, task } = require('gulp'),
  ...
  moveSource = require('./scripts/move/move-src.js'), { moveImages, moveFonts, moveFavicons, moveJSON, moveBlocks, moveSections, movePHP, moveHTML, moveVendor } = require('./scripts/move/move-files.js'),
...
task('default', function(done) {
  ...
  watch(path.join(config.src.vendor, '**', '*'), moveVendor);
});
