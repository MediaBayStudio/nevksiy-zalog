let { src, dest } = require('gulp'),
  ...
  moveVendor = function(cb) {
    src(path.join(config.src.vendor, '**', '*'), { allowEmpty: true })
      .pipe(dest(config.dest.vendor))
      .pipe(dest(path.join('dist', 'vendor')));
    cb();
  };


module.exports = { moveImages, moveFonts, movePHP, moveHTML, moveSections, moveBlocks, moveFavicons, moveJSON, moveVendor };